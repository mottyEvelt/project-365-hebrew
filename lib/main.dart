...
class _YearProgressScreenState extends ConsumerState<YearProgressScreen> {
  ...
  Future<void> _syncWidgetData() async {
    final hebrewDate = await HebrewCalendar.now();
    final dayOfYear = hebrewDate.getDayOfYear();
    final totalDays =
        HebrewCalendar.isLeapYear(hebrewDate.year) ? 355 : 353;
  ...
  Future<HebrewCalendar> _getHebrewDate() async {
    return await HebrewCalendar.now();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: FutureBuilder<HebrewCalendar>(
        future: _getHebrewDate(),
        builder: (context, snapshot) {
          ...
          final hebrewDate = snapshot.data!;
          ...
          final formattedDate = "It's $day$suffix $monthName.";
          ...
          final percentPassed =
              (dayOfYear / totalDays * 100).toStringAsFixed(1);
            ...
            child: LayoutBuilder(
              builder: (context, constraints) {
                ...
                final horizontalUnits =
                    columns + (columns - 1) * spacingFactor;
